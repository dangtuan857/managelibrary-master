...
void borrowBook(vector<hocsinh> &hs, long &sohs, vector<book> &sach, long &sosach, int h);
...
void us3(vector<hocsinh> &hs, long &sohs, vector<book> &sach, long &sosach, int h)
{
	...
	if (c == 1)
	{
		borrowBook(hs, sohs, sach, sosach, h);
	}
}
...
{ // dang dang nhap hoc sinh h
	...
	case 1:
		...
		if (lenh == 1)
		{
			...
			borrowBook(hs, sohs, sach, sosach, h); // Cong
			int lenh;
			cout << "-------------------------------";
